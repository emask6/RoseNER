...
def convert_data_to_json(base_dir, save_data=False, save_dict=False):
    ...
    for idx, x in enumerate(Y):
        ...
        for ids, l in enumerate(label):
            if l.split('-')[0] == 'B':
                if ids == len(label) - 1 or label[ids + 1] == 'O' or label[ids + 1].split('-')[0] == 'B':
                    S_index.append(ids)
                    continue
                if label[ids + 1].split('-')[0] == 'I':
                    B_index.append(ids)
                    continue
            if l.split('-')[0] == 'I':
                if ids == len(label) - 1:
                    E_index.append(ids)
                    continue
                if label[ids + 1] == 'O' or label[ids + 1].split('-')[0] == 'B':
                    E_index.append(ids)
        text = X[idx]
        labels = []
        for index, B in enumerate(B_index):
            labels.append(
                ['T' + str(index)] + [label[B].split('-')[1]] + [B] + [E_index[index] + 1] + [
                    ' '.join(text.split(' ')[B:E_index[index] + 1])])
        for index, S in enumerate(S_index):
            labels.append(
                ['T' + str(index + len(B_index))] + [label[S].split('-')[1]] + [S] + [S + 1] + [
...
if __name__ == '__main__':
    convert_data_to_json('../../data/BC5CDR', save_data=True, save_dict=True)
